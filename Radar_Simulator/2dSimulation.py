...
def calculate_postion_numerically_hyperbola(tx, rx_1, rx_2, drone):
    ...
    result = minimize(
        lambda point: distance_from_ellipses(point, a, b, h, k, c, d, h2, k2),
        initial_guess,
        bounds=bounds,
        method="trust-constr",
    )
...
def calculate_position_numerically_ellipse(tx, rx_1, rx_2, drone, error=True):
    ...
    """
    a1, b1, k1, h1, a2, b2, h2, k2 = calculate_ellipse_params(
        tx, rx_1, rx_2, drone, True
    )
    initial_guess = np.array([0.0, 0.0])
    bounds = ((0, None), (0, None))  # This specifies x >= 0 and y >= 0
    result = minimize(
        lambda point: distance_from_hyperbola_elipse(
            point, a1, b1, h1, k1, a2, b2, h2, k2
        ),
        initial_guess,
        bounds=bounds,
        method="trust-constr",
...
def plot_ellipse_hyperbola(u1, v1, a1, b1, u2, v2, a2, b2, solution=[]):
    ...
    # Plot ellipse
    plt.plot(
        u1 + a1 * np.cos(t_ellipse),
        v1 + b1 * np.sin(t_ellipse),
        label="Range Sum",
        color="blue",
    )
    ...
    # Plot hyperbola
    plt.plot(
        u2 + a2 * np.cosh(t_hyperbola),
        v2 + b2 * np.sinh(t_hyperbola),
        label="Range Diffrence",
        color="red",
    )
    plt.plot(
        u2 - a2 * np.cosh(t_hyperbola), v2 - b2 * np.sinh(t_hyperbola), color="red"
    )  # Second branch
